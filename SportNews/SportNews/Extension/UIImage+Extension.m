...
+ (UIImage *)groupIconWithURLArray:(NSArray *)URLArray bgColor:(UIColor *)bgColor {
    ...
    for (int i = 0; i<URLArray.count;  i++) {
        id content = URLArray[i];
        if ([content isKindOfClass:[UIImage class]]) {
            [imageArray addObject:content];
        }else {
            ...
            if (data) {
                ...
                [imageArray addObject:[UIImage imageNamed:@"默认头像"]];
            }
        }
    }
    
    UIImage *image = [UIImage groupIconWith:imageArray bgColor:SRGB(240)];
