...
@interface GiftBubbleCell()
@property (nonatomic, strong) MLExpression *expression;
@end

@implementation GiftBubbleCell
{
    NSUserDefaults *df;
}
...
- (id)init
{
    if(self)
    {
        self = [[[NSBundle mainBundle] loadNibNamed:@"GiftBubbleCell" owner:self options:nil] objectAtIndex:0];
...
//
    ...
    if ([name isEqualToString:@"管理员："]) {
        ...
        self.btnLevel.frame = CGRectMake(self.btnLevel.frame.origin.x, self.labTitle1.center.y-16, self.btnLevel.frame.size.width, self.btnLevel.frame.size.height);
    } else {
        self.w.constant = 80;
        self.btnLevel.frame = CGRectMake(12, 8, 60, 21.5);
        // [self.btnLevel setTitle:@"" forState:UIControlStateNormal];
        [self.btnLevel setBackgroundImage:[SNMessageCell getMessageLevelImage:self.level] forState:UIControlStateNormal];
        self.btnLevel.frame = CGRectMake(self.btnLevel.frame.origin.x, self.labTitle1.center.y-18, self.btnLevel.frame.size.width, self.btnLevel.frame.size.height);
    }
...
//
    if (message.bubbleAndroidUrl != nil && ![message.bubbleAndroidUrl isEqualToString:@""]) {
        ...
        if (_9fileData != nil) {
            ...
            // FRNinePatchImage *resizableImage = [[FRNinePatchImage alloc]initWithImage:image size:CGSizeMake(self.w2.constant, self.imgView.frame.size.height)];
            UIImage* resizableImage = [image ninePatchImage:2];
            self.imgView.image = resizableImage;
        } else {
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                NSData *data = [NSData dataWithContentsOfURL:[NSURL URLWithString:message.bubbleAndroidUrl]];
